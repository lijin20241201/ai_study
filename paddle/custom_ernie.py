...
# 它的目的是将某个类注册为一个基础模型（base model），使得这个模型可以在框架或库中被自动识别和使用。
@register_base_model
class ErnieModel(ErniePretrainedModel):
    ...
    def forward(self,
        ...
        attention_mask.stop_gradient = True # 禁用梯度
        #  (n,s,d)
        ...
        # transformer编码器的输出,掩码是填充掩码
        encoder_outputs = self.encoder(embedding_output, attention_mask) # (n,s,d)
        sequence_output = encoder_outputs
        # 池化输出
        pooled_output = self.pooler(sequence_output) #(n,d)
...
class ErnieForQuestionAnswering(ErniePretrainedModel):
    ...
    def forward(self,
        ...
        logits = self.classifier(sequence_output) # (n,s,2)
...
class ErnieForTokenClassification(ErniePretrainedModel):
    ...
    def forward(self,
        ...
        logits = self.classifier(sequence_output) # (n,s,num_class)
        return logits
...
class ErnieForMaskedLM(ErniePretrainedModel):
    ...
    def forward(self,
        ...
        outputs = self.ernie(input_ids,
                             ...
                             position_ids=position_ids,
                             attention_mask=attention_mask)
        sequence_output = outputs[0] # (n,s,d)
