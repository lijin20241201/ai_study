...
# 输入数据开始是'[CLS]',使用 '[SEP]' (分割对话内容前一部分，当前部分，后续部分等). 
# 如果有更复杂的对话内容,用'INNER_SEP' 进一步分割.
INNER_SEP = '[unused0]'

class UDCv1(Dataset):
    """
    UDCv1 使用在对话回应选择数据集
    在UDCv1类中作为静态属性（或类变量）存在，它们在类定义时初始化，并在类加载到内存时可用。
    """
    ...
    def read_data(self):
        ...
        with open(data_path, 'r', encoding='utf8') as fin:
            ...
            for line in fin:
                ...
                text_a = arr[1:-1]
                text_b = arr[-1]
                iner_.append([label, text_a, text_b])
            if self._mode=='train':
                iner_=[i for i in iner_ if sum([len(j.split()) for j in i[1]])+len(i[2].split())+3+len(i[1])<=150]
            else:
               iner_= iner_[:1500]
            self.data=iner_

    @classmethod
    def get_label(cls, label): # 类方法
    ...
    def convert_example(cls, example, tokenizer, max_seq_length=512):
        ...
        def _truncate_and_concat(text_a: List[str], text_b: str, tokenizer,
            ...
            tokens_a = []
            for text in text_a: # 遍历其中的每个对话文本
                tokens_a.extend(tokenizer.tokenize(text))
                tokens_a.append(INNER_SEP) # 内部对话分割
            tokens_a = tokens_a[:-1]
        ...
        input_ids, segment_ids = _truncate_and_concat(text_a, text_b, tokenizer,
                                                      max_seq_length)
        # 返回input_ids(text_a+text_b),token_types,label
